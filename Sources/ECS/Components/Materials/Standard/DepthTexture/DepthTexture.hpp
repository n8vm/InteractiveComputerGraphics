...
namespace Components::Materials::SurfaceMaterials {
	class DepthTextureSurface : public Material {
	public:
		static void Initialize(int maxDescriptorSets, std::vector<PipelineParameters> pipelineParameters) {
			DepthTextureSurface::maxDescriptorSets = maxDescriptorSets;
		...
		static void Destroy() {
			for (DepthTextureSurface tbs : DepthTextureSurfaces) tbs.cleanup();
		...
		DepthTextureSurface(std::vector<std::shared_ptr<Scene>> scenes, std::shared_ptr<Textures::Texture> depthTexture, float nearPos, float farPos) : Material() {
			...
			DepthTextureSurfaces.push_back(*this);
		}
		DepthTextureSurface(std::shared_ptr<Scene> scene, std::shared_ptr<Textures::Texture> depthTexture, float nearPos, float farPos) : Material() {
		...
		void render(Scene *scene, std::shared_ptr<Components::Meshes::Mesh> mesh) {
			if (!mesh) {
				std::cout << "DepthTextureSurface: mesh is empty, not rendering!" << std::endl;
				return;
			}
	...
	private:
		...
		static std::vector<DepthTextureSurface> DepthTextureSurface::DepthTextureSurfaces;
		...
		static void createDescriptorSetLayout() {
			...
			if (vkCreateDescriptorSetLayout(VKDK::device, &layoutInfo, nullptr, &descriptorSetLayout) != VK_SUCCESS) {
				throw std::runtime_error("failed to create DepthTextureSurface descriptor set layout!");
			}
		}
		...
		static void setupGraphicsPipeline(std::vector<PipelineParameters> pipelineParameters) {
			...
			auto vertShaderCode = readFile(ResourcePath "MaterialShaders/SurfaceMaterials/DepthTextureSurface/vert.spv");
			auto fragShaderCode = readFile(ResourcePath "MaterialShaders/SurfaceMaterials/DepthTextureSurface/frag.spv");
		...
		void createDescriptorSets(std::vector<std::shared_ptr<Scene>> scenes) {
			for (int i = 0; i < scenes.size(); ++i) {
				...
				depthImageInfo.imageView = depthTexture->getDepthImageView();
				depthImageInfo.sampler = depthTexture->getSampler();
