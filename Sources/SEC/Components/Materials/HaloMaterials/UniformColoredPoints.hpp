...
namespace Components::Materials::HaloMaterials {
	...
	private:
		...
		static std::unordered_map<VkRenderPass, VkPipelineLayout> UniformColoredPoint::pipelineLayouts;
		static std::unordered_map<VkRenderPass, VkPipeline> UniformColoredPoint::pipelines;
		...
		static void setupGraphicsPipeline(std::vector<PipelineParameters> pipelineParameters) {
			...
			/* Override topology parameter to force points */
			for (int i = 0; i < pipelineParameters.size(); ++i) {
				pipelineParameters[i].inputAssembly.topology = VK_PRIMITIVE_TOPOLOGY_POINT_LIST;
			}

			createPipelines(pipelineParameters, shaderStages, { getBindingDescription() }, getAttributeDescriptions(), {descriptorSetLayout},
