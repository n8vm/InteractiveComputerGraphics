...
namespace Components::Materials::SurfaceMaterials {
	class EyeSpaceReflectionSurface : public Material {
	public:
		static void Initialize(int maxDescriptorSets, std::vector<PipelineParameters> pipelineParameters) {
			EyeSpaceReflectionSurface::maxDescriptorSets = maxDescriptorSets;
		...
		static void Destroy() {
			for (EyeSpaceReflectionSurface tbs : EyeSpaceReflectionSurfaces) tbs.cleanup();
		...
		EyeSpaceReflectionSurface(std::vector<std::shared_ptr<Scene>> scenes, std::shared_ptr<Textures::Texture> reflectedSceneTexture) : Material() {
			this->reflectedSceneTexture = reflectedSceneTexture;
			...
			EyeSpaceReflectionSurfaces.push_back(*this);
		}
		EyeSpaceReflectionSurface(std::shared_ptr<Scene> scene, std::shared_ptr<Textures::Texture> reflectedSceneTexture) : Material() {
		...
		void render(Scene *scene, std::shared_ptr<Components::Meshes::Mesh> mesh) {
			if (!mesh) {
				std::cout << "EyeSpaceReflectionSurface: mesh is empty, not rendering!" << std::endl;
				return;
			}
	...
	private:
		...
		static std::vector<EyeSpaceReflectionSurface> EyeSpaceReflectionSurface::EyeSpaceReflectionSurfaces;
		...
		static void createDescriptorSetLayout() {
			...
			if (vkCreateDescriptorSetLayout(VKDK::device, &layoutInfo, nullptr, &descriptorSetLayout) != VK_SUCCESS) {
				throw std::runtime_error("failed to create EyeSpaceReflectionSurface descriptor set layout!");
			}
		}
		...
		static void setupGraphicsPipeline(std::vector<PipelineParameters> pipelineParameters) {
			...
			auto vertShaderCode = readFile(ResourcePath "MaterialShaders/SurfaceMaterials/EyeSpaceReflectionSurface/vert.spv");
			auto fragShaderCode = readFile(ResourcePath "MaterialShaders/SurfaceMaterials/EyeSpaceReflectionSurface/frag.spv");
		...
		std::shared_ptr<Textures::Texture> reflectedSceneTexture;
		...
		void createDescriptorSets(std::vector<std::shared_ptr<Scene>> scenes) {
			for (int i = 0; i < scenes.size(); ++i) {
				...
				imageInfo.imageView = reflectedSceneTexture->getColorImageView();
				imageInfo.sampler = reflectedSceneTexture->getSampler();
