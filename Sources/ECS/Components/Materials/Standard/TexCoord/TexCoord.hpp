...
namespace Components::Materials::SurfaceMaterials {
	...
	public:
		...
		TexCoordSurface(std::vector<std::shared_ptr<Scene>> scenes) : Material() {
	...
	private:
		...
		void createDescriptorSets(std::vector<std::shared_ptr<Scene>> scenes) {
			for (int i = 0; i < scenes.size(); ++i) {
				...
				/* Camera buffer */
				bufferInfos[0].buffer = scenes[i]->getCameraBuffer();
				bufferInfos[0].offset = 0;
				bufferInfos[0].range = sizeof(CameraBufferObject);

				/* Uniform buffer */
				bufferInfos[1].buffer = uniformBuffers[scenes[i]->getRenderPass()];
