...
namespace Components::Materials::SurfaceMaterials {
	...
	public:
		...
		static void Destroy() {
			for (SkyboxSurface ns : SkyboxSurfaces) ns.cleanup();
		...
		SkyboxSurface(std::vector<std::shared_ptr<Scene>> scenes, std::shared_ptr<Components::Textures::Texture> cubemap) : Material() {
			this->cubemap = cubemap;
			createUniformBuffers(scenes, sizeof(UniformBufferObject));
			createDescriptorSets(scenes);

			SkyboxSurfaces.push_back(*this);
		}
		SkyboxSurface(std::shared_ptr<Scene> scene, std::shared_ptr<Components::Textures::Texture> cubemap) : Material() {
		...
		void render(Scene *scene, std::shared_ptr<Components::Meshes::Mesh> mesh) {
			if (!mesh) {
				std::cout << "SkyboxSurface: mesh is empty, not rendering!" << std::endl;
				return;
			}
	...
	private:
		...
		static std::vector<SkyboxSurface> SkyboxSurface::SkyboxSurfaces;
		...
		static void createDescriptorSetLayout() {
			...
			if (vkCreateDescriptorSetLayout(VKDK::device, &layoutInfo, nullptr, &descriptorSetLayout) != VK_SUCCESS) {
				throw std::runtime_error("failed to create SkyboxSurface descriptor set layout!");
			}
		}
		...
		static void setupGraphicsPipeline(std::vector<PipelineParameters> pipelineParameters) {
			...
			auto vertShaderCode = readFile(ResourcePath "MaterialShaders/SurfaceMaterials/SkyboxSurface/vert.spv");
			auto fragShaderCode = readFile(ResourcePath "MaterialShaders/SurfaceMaterials/SkyboxSurface/frag.spv");
		...
		void createDescriptorSets(std::vector<std::shared_ptr<Scene>> scenes) {
			for (int i = 0; i < scenes.size(); ++i) {
				...
				skyboxImageInfo.imageView = cubemap->getImageView();
