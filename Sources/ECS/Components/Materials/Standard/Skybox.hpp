...
namespace Components::Materials::Standard {
	...
	class Skybox : public MaterialInterface {
	public:
		static std::shared_ptr<Skybox> Create(std::string name, PipelineKey pipelineKey) {
			std::cout << "ComponentManager: Adding Skybox \"" << name << "\"" << std::endl;

			auto material = std::make_shared<Material>();
			auto skybox = std::make_shared<Skybox>(pipelineKey);
		...
		Skybox(PipelineKey pipelineKey) {
    ...
    void useTexture(bool useTextureComponent) {
      this->useTextureComponent = useTextureComponent;
    }
	...
	private:
		...
		static void setupGraphicsPipeline() {
			...
			auto vertShaderCode = readFile(ResourcePath "MaterialShaders/Standard/Skybox/vert.spv");
			auto fragShaderCode = readFile(ResourcePath "MaterialShaders/Standard/Skybox/frag.spv");
