...
namespace Components::Materials::SurfaceMaterials {
	class BlinnShadowSurface : public Material {
	public:
		static void Initialize(int maxDescriptorSets, std::vector<PipelineParameters> pipelineParameters) {
			BlinnShadowSurface::maxDescriptorSets = maxDescriptorSets;
		...
		static void Destroy() {
			for (BlinnShadowSurface mat : BlinnShadowSurfaces) mat.cleanup();
		...
		BlinnShadowSurface(std::shared_ptr<Scene> scene, 
			...
			std::shared_ptr<Textures::Texture> BShadowMap) : Material() 
		{
			...
			BlinnShadowSurfaces.push_back(*this);
		}
		BlinnShadowSurface(std::vector<std::shared_ptr<Scene>> scenes, 
			std::shared_ptr<Textures::Texture> LShadowMap,
			std::shared_ptr<Textures::Texture> RShadowMap,
			std::shared_ptr<Textures::Texture> UShadowMap,
			std::shared_ptr<Textures::Texture> DShadowMap,
		...
		void render(Scene *scene, std::shared_ptr<Components::Meshes::Mesh> mesh) {
			if (!mesh) {
				std::cout << "BlinnShadowSurface: mesh is empty, not rendering!" << std::endl;
				return;
			}

			if (descriptorSets.find(scene->getRenderPass()) == descriptorSets.end()) {
				std::cout << "BlinnShadowSurface: Warning: no descriptor set for provided render pass. not rendering!" << std::endl;
				return;
			}
	...
	private:
		...
		static std::vector<BlinnShadowSurface> BlinnShadowSurface::BlinnShadowSurfaces;
		...
		static void createDescriptorSetLayout() {
			...
			if (vkCreateDescriptorSetLayout(VKDK::device, &layoutInfo, nullptr, &descriptorSetLayout) != VK_SUCCESS) {
				throw std::runtime_error("failed to create BlinnShadowSurface descriptor set layout!");
			}
		}
		...
		static void setupGraphicsPipeline(std::vector<PipelineParameters> pipelineParameters) {
			...
			auto vertShaderCode = readFile(ResourcePath "MaterialShaders/SurfaceMaterials/BlinnShadowSurface/vert.spv");
			auto fragShaderCode = readFile(ResourcePath "MaterialShaders/SurfaceMaterials/BlinnShadowSurface/frag.spv");
